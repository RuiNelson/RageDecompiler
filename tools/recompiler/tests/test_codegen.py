...
def test_every_instruction_uses_a_brace_block():
    """Uniform { BEFORE … } per instruction (temps reuse t0; listing is consistent)."""
    ins = {
    ...
    assert '{\n        BEFORE_INSTRUCTION' in lea or '{\n    BEFORE_INSTRUCTION' in lea
...
# --- hand-written multi-instruction patterns ------------------------------

def _seq_move_imm_b_disp(addr, imm, disp, an=0, bl=4):
    """move.b #imm, disp(An) — structure-init shape (imm_disp_store_chain)."""
    return _fn_instr(
        addr, 'move', 'b',
...
def test_pattern_imm_disp_store_chain_custom_block():
    """Registered imm_disp_store_chain replaces the sequence with custom C++."""
    ins = {
        0x100: _seq_move_imm_b_disp(0x100, 1, 0),
        0x104: _seq_move_imm_b_disp(0x104, 2, 1),
        0x108: _fn_instr(0x108, 'rts', None, [], FlowType.RETURN),
    }
    src = Generator(ins, {0x100}).emit_source()
    body = _function_source(src, 'sub_000100')
    assert 'pattern imm_disp_store_chain' in body
    assert body.count('BEFORE_INSTRUCTION') == 3  # 2 stores + rts
    assert 'writeByte' in body
    assert body.count('setNZ') <= 1


def test_pattern_never_matches_across_label():
    """A mid-sequence goto target blocks matching (zero danger)."""
    ins = {
        0x100: _fn_instr(0x100, 'beq', None, [], FlowType.CONDITIONAL,
                         targets=[0x106], bl=2),
        0x102: _seq_move_imm_b_disp(0x102, 1, 0, bl=4),
        0x106: _seq_move_imm_b_disp(0x106, 2, 1, bl=4),
        0x10A: _fn_instr(0x10A, 'rts', None, [], FlowType.RETURN),
    }
    src = Generator(ins, {0x100}).emit_source()
    body = _function_source(src, 'sub_000100')
    assert 'goto L000106' in body
    assert 'L000106:' in body
    assert 'pattern imm_disp_store_chain $000102' not in body


def test_pattern_memfill_long_postinc_custom_loop():
    """Registered memfill_long_postinc emits a hand-written for-loop."""
    n = 4
    ins = {}
    addr = 0x100
    for _ in range(n):
    ...
    src = Generator(ins, {0x100}).emit_source()
    body = _function_source(src, 'sub_000100')
    assert 'pattern memfill_long_postinc' in body
    assert 'for (int _seq = 0; _seq < 4; ++_seq)' in body
    assert body.count('BEFORE_INSTRUCTION') == 2  # loop body + rts
...
def test_custom_pattern_registration():
    """New patterns can be registered and emit arbitrary C++."""
    from tools.recompiler import patterns as pat

    def match_two_nops(instrs):
        if (len(instrs) >= 2
                and instrs[0].mnemonic == 'nop'
                and instrs[1].mnemonic == 'nop'):
            return 2, {}
        return None

    def emit_two_nops(m, live_out):
        return [
            'BEFORE_INSTRUCTION',
            'BEFORE_INSTRUCTION',
            '/* custom double-nop */ (void)0;',
        ]

    custom = pat.Pattern(
        name='test_double_nop',
        doc='unit-test only',
        match=match_two_nops,
        emit=emit_two_nops,
    )
    # Temporarily install just this pattern.
    old = list(pat.PATTERNS)
    pat.PATTERNS[:] = [custom]
    try:
        ins = {
            0x100: _fn_instr(0x100, 'nop', None, [], bl=2),
            0x102: _fn_instr(0x102, 'nop', None, [], bl=2),
            0x104: _fn_instr(0x104, 'rts', None, [], FlowType.RETURN),
        }
        src = Generator(ins, {0x100}).emit_source()
        body = _function_source(src, 'sub_000100')
        assert 'pattern test_double_nop' in body
        assert 'custom double-nop' in body
    finally:
        pat.PATTERNS[:] = old
