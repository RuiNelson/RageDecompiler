...
Output: ``SoR.hpp``, ``SoR-common.hpp``, and one ``SoR-XXX.cpp`` translation
unit per high 12-bit entry-address group. Generation is total — an opcode that
...
# Identifiers a generated function must not take (StreetsOfRage / MegaDriveEnvironment
# members and a few obvious C++ keywords); such a label falls back to sub_….
_RESERVED = {
    'run', 'dispatch', 'unhandledDispatch', 'boot', 'powerOn', 'powerOff',
    'onPowerOn', 'cpuInterruptMask', 'dumpUnhandledDispatchCpuState', 'handleOptionHotkey',
    'StreetsOfRage',
...
class Generator:
    ...
    def _emit_function(self, func):
        out = [f'void StreetsOfRage::{self.fn(func.entry)}(m_long entry_) {{']
    ...
    def _emit_speculative_wrapper(self, addr):
        ...
        owner = self.part.func_of(addr)
        return '\n'.join([
            f'void StreetsOfRage::{self.speculative_fn(addr)}() {{',
    ...
    def _emit_entry_alias_wrapper(self, addr):
        ...
        owner = self.part.func_of(addr)
        return '\n'.join([
            f'void StreetsOfRage::{self.fn(addr)}(m_long entry_) {{',
    ...
    def emit_common_header(self):
        return _COMMON_HEADER_TEMPLATE.format(
            cast_macros=sem.CAST_MACROS.strip())

    @staticmethod
    def _source_group(entry):
        """Return the first three digits of a six-digit ROM entry address."""
        return f'{entry >> 12:03X}'

    def emit_sources(self):
        """Emit source translation units keyed by their three-digit group."""
        ea.set_active_rom(self.rom)
        try:
            return self._emit_source_bodies()
        finally:
            ea.set_active_rom(None)

    def emit_source(self):
        """Return all generated sources together for codegen-level tests."""
        return self.emit_common_header() + '\n'.join(
            self.emit_sources().values())
    ...
    def _emit_source_bodies(self):
        boot = self.fn(self.part.func_of(0x000200))
        boot_group = self._source_group(self.part.func_of(0x000200))
        grouped_parts = {
            boot_group: [
                _SOURCE_RUNTIME.format(
        ...
        disp = ['void StreetsOfRage::dispatch(m_long addr) {', '    switch (addr) {']
        ...
        grouped_parts[boot_group].append(self._emit_call_return_log())
        grouped_parts[boot_group].append('\n'.join(disp))

        for e in self.part.entries:
            if e not in rejected and e not in self._manual_functions:
                grouped_parts.setdefault(self._source_group(e), []).append(bodies[e])

        for addr in sorted(self._entry_aliases):
            if self.part.func_of(addr) not in rejected:
                grouped_parts.setdefault(self._source_group(addr), []).append(
                    self._emit_entry_alias_wrapper(addr))

        for addr in sorted(self._speculative):
            owner = self.part.func_of(addr)
            if addr not in self.part.functions and owner not in rejected:
                grouped_parts.setdefault(self._source_group(addr), []).append(
                    self._emit_speculative_wrapper(addr))

        return {
            group: _SOURCE_FILE_HEADER + '\n\n'.join(grouped_parts[group]) + '\n'
            for group in sorted(grouped_parts)
        }
...
class StreetsOfRage : public MegaDriveEnvironment {{
    public:
    explicit StreetsOfRage(const std::string &romPath,
...
StreetsOfRage::StreetsOfRage(const std::string &romPath, VDP::Synchronization sync,
                         VDP::Scaling scaling, std::uint16_t remoteAccessPort)
    : MegaDriveEnvironment(sync, scaling, VDP::HardwareSpriteLimit, remoteAccessPort) {{
    loadROM(romPath);
}}

void StreetsOfRage::unhandledDispatch(m_long addr) {{
    // Delegated to reportUnhandledDispatch so host diagnostics stay in non-generated code.
    reportUnhandledDispatch(addr);
}}

void StreetsOfRage::serviceIRQ() {{
